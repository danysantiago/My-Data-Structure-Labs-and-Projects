 ...
 *
 */
public class NodePositionList3<T> implements PositionList<T> {

	private DNode<T> header, trailer; 
	private int count;
	PositionListIteratorMaker<T> iteratorMaker;
	
	public NodePositionList3() {
		header = new DNode<T>(null, null, null, this); 
		trailer = new DNode<T>(null, header, null, this);
		header.setNext(trailer); 
		count = 0;
		iteratorMaker = new PLFowardIterator<T>();
	}
	
	public NodePositionList3(PositionListIteratorMaker<T> iteratorMaker) {
		this();
	...
	public Iterable<Position<T>> positions() {
		NodePositionList3<Position<T>> pList = new NodePositionList3<Position<T>>(); 
		try { 
			if (!this.isEmpty()) { 
	...
	private static class DNode<T2> implements Position<T2> {
        ...
        private NodePositionList3<T2> myList;     // list it belongs to
        
        public DNode(T2 e, DNode<T2> p, DNode<T2> n, NodePositionList3<T2> list) { 
        	element = e; 
        	next = n; 
        	prev = p; 
        	myList = list; 
		...
		public void setList(NodePositionList3<T2> n) { 
			myList = n; 
		}
		public NodePositionList3<T2> getList() { 
			return myList; 
		}
